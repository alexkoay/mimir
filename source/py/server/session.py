...
class Session:
    ...
    async def run(self):
        self.log.info('(%s) Connected to %s (%s)', self._id, self._socket.request_headers.get('X-Real-IP', self._socket.remote_address[0]), self._socket.request_headers.get('User-Agent', 'unknown'))
        error = False
